...
final class WorkflowRunController extends Controller
{
    ...
    public function retry(WorkflowRun $workflowRun,WorkflowEngine $engine){$new=$engine->start($workflowRun->workflow,$workflowRun->trigger,(array)$workflowRun->input,'execute',$workflowRun->id);return redirect()->route('workflow-runs.show',$new)->with('status','Workflow retry started as a new immutable run.');}
    public function resume(WorkflowRun $workflowRun){abort_if(in_array($workflowRun->status,['completed','failed','cancelled'],true),409,'This workflow run is terminal. Use Retry to create a new immutable run.');ContinueWorkflowRunJob::dispatch($workflowRun->id);return back()->with('status','Workflow run was re-queued.');}
}

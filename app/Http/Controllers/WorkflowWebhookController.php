...
{
    public function __invoke(Request $request,Workflow $workflow,string $secret,WorkflowEngine $engine)
    {
        ...
        $payload=$request->all();if($payload===[])$payload=['raw'=>$request->getContent()];$run=$engine->start($workflow,'webhook',$payload);return response()->json(['ok'=>true,'run_id'=>$run->id],202);
    }
}

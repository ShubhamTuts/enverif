...
?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?=htmlspecialchars($title)?> · Enverif Docs</title><meta name="description" content="Complete Enverif documentation for installation, shared hosting, revenue agents, workflows, email automation, plugins and contributors."><link rel="icon" href="/assets/enverif-mark.svg"><link rel="stylesheet" href="/assets/docs.css"></head><body><div class="layout"><aside><a class="brand" href="/overview"><img src="/assets/enverif-mark.svg" alt=""><span><b>Enverif</b><small>Docs · by Codefreex</small></span></a><div class="search"><input type="search" placeholder="Filter docs…" oninput="document.querySelectorAll('[data-nav]').forEach(a=>a.hidden=!a.textContent.toLowerCase().includes(this.value.toLowerCase()))"></div><nav><?php foreach($groups as $g=>$items):?><div class="nav-group"><strong><?=htmlspecialchars($g)?></strong><?php foreach($items as $slug=>$label):?><a data-nav class="<?=$slug===$key?'active':''?>" href="/<?=htmlspecialchars($slug)?>"><?=htmlspecialchars($label)?></a><?php endforeach;?></div><?php endforeach;?></nav><div class="aside-foot"><a href="https://github.com/ShubhamTuts/enverif">GitHub repository ↗</a><a href="/contributing">Contribute to Enverif</a><span>MIT · Enverif by Codefreex</span></div></aside><main><header><button class="mobile" onclick="document.body.classList.toggle('nav-open')">☰</button><span><?=htmlspecialchars($group)?> <i>/</i> <?=htmlspecialchars($title)?></span><div><a href="/installation">Install</a><a class="github" href="https://github.com/ShubhamTuts/enverif">GitHub ↗</a></div></header><article><?=renderMd($markdown)?><footer><span>Was this useful? Open a GitHub issue for missing documentation.</span><span>Enverif by Codefreex</span></footer></article></main></div></body></html>

...
<div class="form-group"><label class="form-label">{{ __('ui.api_key') }}</label><input class="field" type="password" name="api_key" autocomplete="off" @if(!$connection->exists)required @endif><div class="help">{{ $connection->exists ? __('ui.credential_kept') : '' }}</div></div>
